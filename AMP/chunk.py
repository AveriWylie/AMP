...
A few things to note. You need to download blocks.json for version 1.20.1 from the 
...
class Chunk:
    ...
    # load block state lookup once at class level, same pattern as Bot loading mc_versions.txt
    _blocks_json = json.loads((Path(__file__).parent / "blocks.json").read_text())

    # map every state id to its block name once at startup
    _state_to_block = {}
    for _block in _blocks_json:
        for _state in _block["states"]:
            _state_to_block[_state["id"]] = _block["name"]
