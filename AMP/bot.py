...
class Connection:
    ...
    def _read_varint_from_socket(self) -> int:
        ...
        while True:
            ...
            # high is 0 then break other wise read moire
...
class Bot:
    ...
    """
    --------------------------------------------------------------------------------------------
    Function Header - Chunk and block update handlers
    --------------------------------------------------------------------------------------------
    ...
    Block update patches a single block into the already-stored chunk so world state stays
    accurate without a full re-parse. Block position is packed into a single long as
    ...
    def _handle_chunk(self, payload):
        ...
        self._world_state["map"][(cx, cz)] = Chunk(chunk_data)
    ...
    Derives weight inline from self._input_mode directly.
    --------------------------------------------------------------------------------------------
    """
