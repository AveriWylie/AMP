...
def test_window_items_replaces_player_inventory_and_names_items():
    ...
    bot._world_tracker._on_packet(bot.play_ids["window_items"], payload)
...
def test_set_slot_adds_and_removes_hotbar_item():
    ...
    bot._world_tracker._on_packet(bot.play_ids["set_slot"], prefix + _slot(799))
    assert bot._world_state["inventory"]["slots"][38]["name"] == "diamond_pickaxe"

    bot._world_tracker._on_packet(bot.play_ids["set_slot"], prefix + _slot())
...
def test_stale_window_snapshot_does_not_overwrite_newer_slot_update():
    ...
    bot._world_tracker._on_packet(bot.play_ids["set_slot"], update)
    stale = b"\x00" + Connection._encode_varint(4) + Connection._encode_varint(1)
    stale += _slot() + _slot()

    bot._world_tracker._on_packet(bot.play_ids["window_items"], stale)
...
def test_slot_decoder_skips_compound_nbt_before_next_slot():
    ...
    first, offset = bot._protocol_adapter._decode_slot(payload, 0)
    second, offset = bot._protocol_adapter._decode_slot(payload, offset)
