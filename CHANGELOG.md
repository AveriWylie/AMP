...
- Provides guided and autonomous Claude planning with validated structured commands and server-confirmed execution feedback.
- Reads Anthropic credentials from the environment or an ignored `.env` file.
- Keeps transport, world state, gameplay, lifecycle, execution, and planning in separate modules behind the `Bot` façade.
